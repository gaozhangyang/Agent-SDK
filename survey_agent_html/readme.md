...
| 模块 | 状态 | 说明 |
|------|------|------|
| **fetch_arxiv.py** | ✅ 通过 | 成功从arXiv获取论文 |
| **download_pdf.py** | ✅ 通过 | 成功下载PDF文件 |
| **screen_papers.py** | ✅ 通过 | 成功筛选论文 |
| **extract_text.py** | ✅ 通过 | 成功提取PDF文本 |
| **SDK 服务** | ✅ 运行中 | 健康检查通过 (http://127.0.0.1:3890) |
| **API Server** | ✅ 运行中 | 所有端点测试通过 (http://127.0.0.1:8000) |
| **前端页面** | ✅ 可访问 | http://localhost:8000 |
| **端到端** | ✅ 通过 | 成功抓取5篇论文，筛选3篇，分析3篇 |
| **Session 恢复** | ✅ 通过 | 正确加载之前的 Trace/State |
...
# 2. 启动 API Server (端口 8000)
cd survey_agent_html
  ...
  "topics": [
    {
      ...
      "keywords": ["LLM", "transformer", "RLHF", "RAG"],
      "arxiv_categories": ["cs.CL", "cs.AI"],
      "max_papers_per_day": 3
    }
  ],
  "global_settings": {
    "pdf_download_dir": "<项目目录>/survey_agent_html/data/pdfs",
...
**注意**: `pdf_download_dir` 必须指向 `survey_agent_html/data/pdfs` 目录，否则 SDK 无法找到 PDF 文件。

### AGENT.md 运行时配置

Survey Agent 支持通过 `.agent/AGENT.md` 文件进行运行时配置。配置内容写在 md 文件里的 ```json 代码块中：
...
| `/api/papers/{arxiv_id}/feedback` | POST | 保存用户对论文的评分/标注 |
| `/api/config/topics` | PUT | 修改用户兴趣配置 |
