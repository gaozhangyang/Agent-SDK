...
- **验证循环**：执行 script 后，通过 `prompts/verifier.md` 调用 verifier LLM 验证结果是否满足目标。**verifier 和 revise 合并为一次 llm_call**，输出包含 `pass`、`feedback`、`revised_goal` 字段。验证失败则更新 history 并重试（最多 `MAX_VERIFY_RETRY` 次，默认 3 次）。
- **马尔可夫结构**：history 退化为只保留上一时刻（last_script、last_feedback、last_revised_goal），不累积。`original_goal` 始终不变，revised_goal 只作为 prompt 注入变量，不写文件。
...
def execute_decompose(goal_dir, goal, subtasks, depth, permissions):
    ...
    # 将 decomposition_id 写入父节点 meta.json（使 Recovery 校验生效）
    parent_meta = read_meta(goal_dir)
    parent_meta["decomposition_id"] = decomposition_id
    write_meta(goal_dir, parent_meta)
...
└── verifier.md        # 验证器 prompt（合并了 revise，同时输出 pass/feedback/revised_goal）
```
...
2. **修改特定行为**: 
   ...
   - 修改 `prompts/verifier.md` 可以改变结果验证逻辑（包含目标修订）
