...
> meta-agent 是一个递归函数：probe 退化为确定性操作，决策直接解决或分解子任务。直接解决模式：LLMCall 生成 script + plan，执行后通过 verifier 验证结果，验证失败则更新 history 并重试（马尔可夫结构，最多 MAX_VERIFY_RETRY 次）。分解执行模式：递归执行子节点，子节点失败时重试（最多 MAX_RETRY 次），然后通过 merge_results 直接合并子节点 results + observations。script.py 以标准 Python 执行（open、subprocess 等），必须输出 RESULT + OBSERVATIONS 结构；results.md 写入即成符号事实；目录树即任务树，文件系统即执行状态，Recovery 只读 results.md 判断失败。
