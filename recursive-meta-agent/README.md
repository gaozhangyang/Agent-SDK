...
- **代码生成**：由 `prompts/code_generator.md` 驱动，要求模型**同时输出**一段 Python 脚本（`\`\`script` 块）和执行计划（`\`\`plan` 块），禁止 `<tool_code>` 等工具调用标记；脚本内仅使用注入的 `goal_dir`、`read`、`write`、`bash`、`llm_call`。鼓励在脚本中调用已有 skills/tools（如通过 `bash(...)`），路径未知时先探索或结合上一轮 error 反馈再试。
- **验证循环**：执行 script 后，通过 `prompts/verifier.md` 调用 verifier LLM 验证结果是否满足目标。如果验证失败，通过 `prompts/revise.md` 修订目标，然后重新执行（最多 `MAX_VERIFY_RETRY` 次，默认 3 次）。
...
- **路径与重试**：运行时向脚本注入 `goal_dir`（当前节点目录），脚本应基于其拼路径，避免硬编码。若存在 `error.md`，其内容会作为「Error from previous attempt」注入下一轮生成，便于模型修正路径或增加探索逻辑。
- **结果写入**：若脚本未写入 results.md，执行器会补写一条完成记录；若脚本异常退出，会写 error.md 并写 escalated 的 results.md（含完整 console），保证每节点都有可追溯的结果记录。
...
def execute_decompose(goal_dir, goal, subtasks, depth, permissions):
    ...
    # 聚合子节点结果：直接合并各子节点的 results.md，不再调用 LLM
    merge_results(goal_dir, subtasks, goal, logger)
    # results.md 由 merge_results 直接写入
...
results.md 以**可读的控制台风格**呈现：首行为执行状态，便于程序解析；其余为 `--- result ---` / `--- console ---` 区块，便于人类查看。程序通过 `parse_results_content()` 解析，兼容本格式与旧版 JSON 格式。
...
包含：四个原语、probe-first、直接执行与分解执行、递归子节点、聚合 results.md、decomposition_id 校验、确定性依赖校验层、基于文件系统的 Recovery、深度限制、error.md 写入、全局 trace / terminal / state 记录；script 在子进程中执行（script_runner.py）、控制台输出完整进 results.md、results.md 控制台风格（首行 status + result/console 区块）、每级目标名前编号便于调试。
...
> meta-agent 是一个递归函数：probe 以最小代价理解任务形状，决策直接解决或分解子任务。直接解决模式：LLMCall 生成 script + plan，执行后通过 verifier 验证结果，验证失败则通过 revise 修订目标并重试（最多 MAX_VERIFY_RETRY 次）。分解执行模式：递归执行子节点，然后通过 merge_results 直接合并子节点结果（不再调用 LLM）。script.py 以确定性代码 + LLMCall 的混合脚本执行，results.md 写入即成符号事实；目录树即任务树，文件系统即执行状态，Recovery 是树扫描而非状态回滚，四个原语组合实现一切能力。
...
9. **script 子进程执行**: 通过 `src/script_runner.py` 在子进程中执行 script.py，隔离崩溃；控制台输出完整写入 results.md。超时由环境变量 `SCRIPT_RUN_TIMEOUT`（秒，默认 600）控制。
...
├── code_generator.md    # 代码生成 prompt（直接执行）：同时输出 script + plan、禁止 tool 标记、鼓励 skills/tools、路径用 goal_dir、缺信息时先探索或结合 error 再试
├── aggregator.md        # 结果聚合 prompt（分解执行模式，已被 merge_results 替代）
├── decision.md         # 决策 prompt（直接解决还是分解；可先分解"探索环境"子目标）
├── decomposer.md       # 任务分解 prompt
├── file_probe.md       # 文件探测 prompt（Probe 阶段）
├── verifier.md        # 验证器 prompt（验证执行结果是否满足目标）
└── revise.md          # 修订 prompt（根据验证反馈修订目标）
```
...
2. **修改特定行为**: 
   ...
   - 修改 `prompts/code_generator.md` 可以改变代码生成风格与路径/探索/error 反馈策略
   - 修改 `prompts/aggregator.md` 可以改变结果聚合逻辑（已不常用）
   - 修改 `prompts/verifier.md` 可以改变结果验证逻辑
   - 修改 `prompts/revise.md` 可以改变目标修订逻辑
