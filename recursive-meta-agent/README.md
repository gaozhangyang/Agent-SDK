...
│   │                               # 新增配置项：context_compression_threshold
│   └── sessions/                   # 每次运行摘要索引
│
└── {goal}/                          # 根节点，对应用户原始问题
    ├── goal.md                      # 任务描述，只读，由父节点或用户写入
    ├── global_context.md           # 全局 context，累积所有节点的 observations 和 console
...
context_compression_threshold: 120000  # global context 压缩阈值，建议为 contextBudget.total 的 60%
maxRetry: 3
...
def probe(goal_dir, goal, permissions, depth):
    ...
    # 根节点的 global context（所有节点的 observations 累积）
    global_context = read(f"{root_goal_dir}/global_context.md")
    if global_context:
...
Probe 是每个节点执行的第一步，**退化为纯确定性操作**。不再调用 LLM 决定读取哪些文件，而是按固定规则读取：goal.md（当前节点）、{rootGoalDir}/global_context.md（所有节点累积的 observations）、results.md（如存在，retry 场景）、memory.jsonl 最近 5 条。探索环境的能力完全下放给 script.py。

### 直接执行

- **代码生成**：由 `prompts/code_generator.md` 驱动，要求模型**同时输出**一段 Python 脚本（`\`\`script` 块）和执行计划（`\`\`plan` 块），禁止 `<tool_code>` 等工具调用标记。脚本使用**标准 Python**（open、subprocess、os 等），执行器仅注入 **goal_dir** 并将当前工作目录设为 goal_dir。鼓励在脚本中通过 `subprocess.run(...)` 等调用已有 skills/tools，路径未知时先探索或结合上一轮 feedback 再试。
...
- **脚本执行**：在**子进程**中通过 `script_runner.py` 执行 `script.py`（仅注入 goal_dir 并 chdir 到 goal_dir），子进程崩溃或超时不会拖垮主进程。子进程的 **stdout/stderr 完整捕获**，作为「控制台输出」写入 results.md 的 `--- console ---` 区块（成功与失败均保留，报错信息一并纳入）。
- **路径与重试**：脚本内基于 `goal_dir` 拼路径（如 `os.path.join(goal_dir, "output", "result.json")`），避免硬编码。若上次执行失败，feedback 信息会注入下一轮生成，便于模型修正路径或增加探索逻辑。
- **script.py 输出规范**：script.py 是纯执行者，只 print 可观测的事实，不做成功/失败的语义判断。使用结构化标记：
  - `[DONE] <完成的操作描述>` - 每次文件读写或解析操作后输出
  - `[FAILED] <失败的操作描述>` - 操作失败时输出
  - `[DATA:<label>] <<<\n<完整数据内容>\n>>>` - 输出完整的中间数据
- **结果写入**：script.py 执行后写入 results.md，**只写 result 和 console，不写 observations**：
  ```
  status: pending
...
def execute_decompose(goal_dir, goal, subtasks, depth, permissions):
    ...
    for level_tasks in levels:
        for task in level_tasks:
            ...
            # 子节点完成后，把 observations 写入根目标的 global_context.md
...
results.md 写入分为两个阶段：
- **阶段一**：script.py 执行后写入，只包含 `result` 和 `console`，不包含 `observations`
- **阶段二**：verifier 分析后补全，填写 `status` 和 `observations`
...
--- observations ---
（verifier 从 console 原始数据中提炼出的具体信息，直接写入父节点 context.md）
```

**observations 的本质**：verifier 从 console 原始数据中提炼出对父节点解决问题有用的具体信息。

不是描述执行了什么操作，而是从数据中提取出可直接使用的具体细节：
...
每个根目标目录下有 `global_context.md`，累积存储所有子节点的 observations 和 console：
...
# From: survey/2_fetch_papers/1_call_arxiv_api
## observations
...
所有节点 probe 阶段读取 global context，拼接进 prompt 作为 LLM 输入的前缀。利用 LLM prefix cache 机制，global context 作为每次调用的前缀，缓存命中后成本极低。

超过压缩阈值时自动触发压缩，保留所有 observations，优先裁剪 console 部分。
...
> meta-agent 是一个递归函数：probe 退化为确定性操作，决策直接解决或分解子任务。直接解决模式：LLMCall 生成 script + plan，执行后通过 verifier 验证结果，验证失败则更新 history 并重试（马尔可夫结构，最多 MAX_VERIFY_RETRY 次）。script.py 使用 print 标记 `[DONE]`/`[FAILED]`/`[DATA:<label>]` 输出可观测事实，results.md 写入分两阶段：script.py 写入 result + console，verifier 分析后补全 status 和 observations。分解执行模式：递归执行子节点，子节点失败时重试（最多 MAX_RETRY 次），然后通过 merge_results 直接合并子节点 results + observations。**所有节点的 observations 累积写入 global_context.md，所有节点 probe 阶段均可读取**，超过压缩阈值时自动触发压缩。script.py 以标准 Python 执行（open、subprocess 等）；results.md 写入即成符号事实；目录树即任务树，文件系统即执行状态，Recovery 只读 results.md 判断失败。
...
├── verifier.md         # 验证器 prompt（输出 pass/feedback + observations）
└── compressor.md       # 压缩器 prompt（压缩 global context，保留 key facts）
```
