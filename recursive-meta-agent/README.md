...
- **代码生成**：由 `prompts/code_generator.md` 驱动，要求模型**只输出**一段 Python 脚本（`\`\`python` 块），禁止 `<tool_code>` 等工具调用标记；脚本内仅使用注入的 `goal_dir`、`read`、`write`、`bash`、`llm_call`。鼓励在脚本中调用已有 skills/tools（如通过 `bash(...)`），路径未知时先探索或结合上一轮 error 反馈再试。
...
├── code_generator.md    # 代码生成 prompt（直接执行）：只输出 script、禁止 tool 标记、鼓励 skills/tools、路径用 goal_dir、缺信息时先探索或结合 error 再试
